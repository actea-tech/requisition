-- requisition_kind (added in 0049) is requester-editable the same way
-- requisition_type already is — add it to enforce_field_write_scope()'s
-- requester_fields_changed comparison so it's covered by the same
-- enforcement, not merely left unwatched by this trigger (RLS + the
-- column-level GRANT already gate who/when at the table level; this keeps
-- the trigger's own bookkeeping consistent with that).
